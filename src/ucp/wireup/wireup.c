...
static void ucp_wireup_log_ep_lanes(ucp_worker_h worker,
                                    const ucp_ep_config_key_t *key,
                                    ucp_rsc_index_t cm_index,
                                    ucp_ep_h ep)
{
    ...
    size_t len, total_width, types_len;
    const char *tl_name, *dev_name, *sysdev_name, *ep_type;
    char title_buf[96];
    char types_buf[128];
    char dev_buf[128];
    ...
    for (lane = 0; lane < key->num_lanes; ++lane) {
        ...
        if ((lane != key->cm_lane) &&
            ...
            sysdev_name = ucs_topo_sys_device_get_name(
                    context->tl_rscs[key->lanes[lane].rsc_index]
                            .tl_rsc.sys_device);
            if (sysdev_name != NULL) {
                snprintf(dev_buf, sizeof(dev_buf), "%s (%s)", dev_name,
                         sysdev_name);
            } else {
                snprintf(dev_buf, sizeof(dev_buf), "%s", dev_name);
            }
        } else {
            snprintf(dev_buf, sizeof(dev_buf), "%s", dev_name);
        }
        ...
        for (lt = UCP_LANE_TYPE_FIRST; lt < UCP_LANE_TYPE_LAST; ++lt) {
            if (types_union & UCS_BIT(lt)) {
                if (types_len > 0) {
                    types_len += snprintf(types_buf + types_len,
                                          sizeof(types_buf) - types_len, ", ");
                }
                types_len += snprintf(types_buf + types_len,
                                      sizeof(types_buf) - types_len, "%s",
